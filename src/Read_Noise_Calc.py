...
class ReadNoiseCalc:
    ...
    def calc_read_noise(self):
        ...
        if self.em_mode == 1:
            ...
            tab_name = 'planilhas/RN_PA' + str(int(self.preamp)) + 'B' + str(int(self.binn)) + 'HSS' + str(int(self.hss)) + '.xlsm'
            df = pd.read_excel(tab_name) 
            columns = pd.DataFrame(df)
            column_noise = columns(tab_name, 'Noise (e-)')[0:11]
            column_em_gain = columns(tab_name, 'EM Gain')[0:11]
